/**
 * 迁移脚本：为 submitted_tasks 表添加驳回次数字段
 * @Author: assistant
 * @Date: 2025-01-11
 * @Description: 添加 reject_times 字段到 submitted_tasks 表
 */
...
async function migrate() {
  ...
  try {
    logger.info('开始执行迁移：添加驳回次数字段到 submitted_tasks 表');
    ...
    logger.info('迁移执行完成：驳回次数字段已添加到 submitted_tasks 表');
    return true;
  } catch (error) {
    // 回滚事务
