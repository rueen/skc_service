...
6. **奖励计算逻辑**
   - 完成任务：审核通过即为完成任务
   - 首次完成任务：会员第一次审核通过的任务
   - 邀请奖励：会员首次完成任务后，该会员的邀请人（如果有的话）获得固定金额的奖励，奖励金额配置在系统配置中`invite_reward_amount`
   - 会员非首次完成任务后，该会员所在群组的群主可以获得任务奖励(reward)的固定比例，群主收益率配置在系统配置中 `group_owner_commission_rate`
   - 任务审核通过后，该会员即可获得任务详情配置的任务奖励(reward)，系统自动创建任务收益账单
   - 收益结算至会员账户余额
   举例：
   - 假设任务奖励为：100，群主收益率为：10%，邀请奖励金额为：5，会员b为会员a的邀请人，会员c为会员a和会员b的群主。
   - 会员a首次完成任务后获得任务奖励100，会员b获取邀请奖励5，会员c获取群主收益0
   - 会员a非首次完成任务后获取任务奖励100，会员c获取群主收益100*10%

7. **账单(Bill)**:
   - 类型: 提现(withdrawal)、任务收益(task_income)、邀请奖励(invite_reward)、群主奖励(group_reward)
