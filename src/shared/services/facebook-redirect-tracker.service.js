...
class FacebookRedirectTrackerService {
   ...
   * @returns {Promise<Array>} 跟踪结果数组
   */
  async batchTrackRedirects(urls, options = {}) {
    const { concurrency = 5 } = options;
    
    logger.info(`[FB-REDIRECT] 🔥 开始批量重定向跟踪: ${urls.length} 个URL, 并发度: ${concurrency}`);
    ...
    for (let i = 0; i < urls.length; i += concurrency) {
      ...
      logger.info(`[FB-REDIRECT] 📦 处理批次 ${Math.floor(i / concurrency) + 1}, 包含 ${batch.length} 个URL`);
      
      const batchPromises = batch.map(async (url, index) => {
        try {
          const result = await this.trackRedirect(url);
    ...
    logger.info(`[FB-REDIRECT] 🎉 批量重定向跟踪完成: ${successCount} 成功, ${failCount} 失败`);
  ...
  getStats() {
    return {
      ...
      proxyHost: this.proxyHost || 'N/A',
      proxyPort: this.proxyPort || 'N/A',
